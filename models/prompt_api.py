template_map = {
'judge_domain': '''
Tugas Anda adalah menentukan domain kueri pengguna. 

ATURAN KETAT:
- finance: HANYA untuk harga saham, market cap, dividend, dan laporan keuangan korporat. Jika kueri tentang produk (misal: "Fitur Microsoft Office"), masukkan ke "open".
- movie: HANYA untuk info film, aktor, sutradara, dan penghargaan Oscar.
- open: Gunakan ini untuk pengetahuan umum, berita produk, teknologi, sejarah, atau jika kueri tidak cocok dengan API database mana pun.

Contoh Kasus Salah:
Query: "Berapa bahasa di Microsoft Office 2019?"
Domain: open (BUKAN finance, karena ini tentang software, bukan saham MSFT)

Query: {query_str}
Domain:''',

# 2. MASTER API PROMPT (MENGGABUNGKAN LOGIKA MOVIE, FINANCE, MUSIC, & SPORTS)
'api_prompt': '''
You are an API assistant. Based on the user query, generate the correct API call(s) using the schemas below.

--- MOVIE DOMAIN ---
- get_movie(movie_name, condition)[key_name]
- get_person(person_name, condition)[key_name]
- get_movie_person_cast(movie_key, person_key, condition)
- get_movie_person_oscar(movie_key, person_key, condition)
Keys: title, release_date, budget, revenue, rating, genres, character, winner.

--- FINANCE DOMAIN ---
- get_stock_price(stock_name, time)[key_name] (Keys: open, high, low, close, volume)
- get_stock_pe_ratio(stock_name)
- get_stock_market_cap(stock_name)
- get_stock_dividend(stock_name)[key_name] (Keys: date, value)

--- MUSIC DOMAIN ---
- get_person(person_name)[key_name] (Keys: birth_place, birth_date, members, lifespan, music)
- get_song(song_name)[key_name] (Keys: author, release_country, release_date)
- get_grammy_person(person_name)[key_name] (Keys: award_count, award_date)

--- SPORTS DOMAIN ---
- get_soccer_next_game(name)
- get_soccer_last_game(name)
- get_nba_on_date(name, date)

RULES:
- Only output the API call lines.
- Use "None" if the API is not applicable.
- If multiple calls are needed, provide them in separate lines.

CONTOH NEGATIF (SANGAT PENTING):
Query: "Siapa CEO Microsoft?" -> Answer: None (Gunakan Open Domain)
Query: "Berapa bahasa yang didukung Office 2013?" -> Answer: None (Gunakan Open Domain)
Query: "Kapan Windows 11 dirilis?" -> Answer: None (Gunakan Open Domain)

HANYA gunakan API jika pertanyaan spesifik menanyakan atribut yang ada dalam database (harga saham, market cap, pemenang Oscar, dll). Jika tidak yakin, jawab: None.

Query: {query_str}
Answer:''',

# 3. TEMPLATE JURI & PENGECEKAN (EXISTING)
'template_judge':'''
...
#Examples:
question: where did the ceo of salesforce previously work? 
answer: none

question: how much did voyager therapeutics's stock rise?
...
Answer whether the question is based on false prepositions or assumptions, output 'invalid question'. 
...
# 4. TEMPLATE DOMAIN SPESIFIK (UNTUK REFERENSI)
'sports_prompt': '''
You are given a query about sport domain...
(Include sports instructions here if needed by other logic)
Query: {query_str}
Answer:
''',

'movie_prompt': '''
You are given a query about movies...
Query: {query_str}
Answer:
'''
}
